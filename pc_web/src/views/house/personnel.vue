<template>
  <div class="main-container">
    ...
    <div class="bg">
      <div class="navbar">
        <span
          :class="{ current: isCurrent == `in` }"
          @click="changeCurrent(`in`)"
          >已 入 住 用 户</span
        >
        <span
          :class="{ current: isCurrent == `house` }"
          @click="changeCurrent(`house`)"
          >待 添 加 房 间</span
        >
        <span
          :class="{ current: isCurrent == `pay` }"
          @click="changeCurrent(`pay`)"
          >待 缴 纳 押 金</span
        >
        <span
          :class="{ current: isCurrent == `out` }"
          @click="changeCurrent(`out`)"
          >退 房 用 户</span
        >
      </div>
        ...
        <div class="bottom">
            ...
            <el-table-column prop="name" label="头像" width="100">
              <template slot-scope="scope">
                <el-avatar
                  ...
                  :src="`${targetUrl}/view/Avatar/${scope.row.avatar}`"
                ></el-avatar>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="180">
              <template slot-scope="scope">
                ...
                <template v-if="isCurrent == `house`">
                  <el-button
                    @click="openAddHouseDrawer(scope.row)"
                    size="mini"
                    type="primary"
                    :disabled="isAdd"
                    >添加</el-button
          ...
          <div class="drawer">
            <el-drawer
              ...
              :show-close="false"
              title="修改租房信息"
              :visible.sync="houseDrawer"
              :destroy-on-close="true"
              size="50%"
            >
              <component
                :is="updateHouseComponent"
                @afterUpdateHouse="afterUpdateHouse"
                @changeUpdateHouseComponent="changeUpdateHouseComponent"
                :user="user"
                :house="houseData"
          ...
          <div class="drawer">
            <el-drawer
              ...
              :show-close="false"
              title="添加杂费信息"
              :visible.sync="basicDrawer"
              :destroy-on-close="true"
              size="50%"
            >
              <update-basic
                :table-data="miscellaneousData"
                :user="user"
                @deleteMiscellaneous="deleteMiscellaneous"
                @afterUpdateBasic="afterUpdateBasic"
                :isDel="isDel"
                :isAdd="isAdd"
              ></update-basic>
            </el-drawer>
          ...
          <div class="drawer">
            <el-drawer
              ...
              :wrapperClosable="false"
              :show-close="false"
              :visible.sync="addHouseDrawer"
              :destroy-on-close="true"
              size="50%"
            >
              <login-house
                :user="user"
                @closeAddHouseDrawer="closeAddHouseDrawer"
              ></login-house>
            </el-drawer>
          </div>
